...
export function loadJSON(filename, d3Options) {
  return getURL(filename).then((url) =>
    url == "" ? returnEmpty([]) : d3.json(url, d3Options)
  );
}

export function loadCSV(filename, d3csvOptions) {
  return getURL(filename).then((url) =>
    url == "" ? returnEmpty([]) : d3.csv(url, d3csvOptions)
  );
}
...
onAuthStateChanged(auth, (user) => {
  if (authcb) {
    // console.log("Calling call back", user);
    authcb(user);
  }
  if (user) {
    //logged in
